...
function init(){
	...
	if(is_agent()){
		...
		if(__SCRIPT__ == 'index.php' && config('mod.installed')){
			...
			if(config('site.maintenance.pages')){
				...
				if(in_array($init['__DISPLAY__'], $maint)){
			...
			if(isset($_SERVER['HTTP_REFERER'])){
				...
				if($url[0] == site_url('mod.php') || url() == site_url('mod.php')) {
					...
					if(config('mod.installed')){
						if(template_file()){
							$init['__DISPLAY__'] = template_file();
						}else{
							$init['__DISPLAY__'] = $err404;
						}
	...
	if(!defined('__DISPLAY__')) define('__DISPLAY__', __SCRIPT__, true);
...
if(is_agent()){
	...
	}else if(__SCRIPT__ == 'index.php'){ /** 载入模板文件 */
		/** 如果系统未安装则跳转到安装页面 */
	...
	WebSocket::on('open', function($event){
		...
		if(($obj == 'mod' || is_subclass_of($obj, 'mod')) && (method_exists($obj, $act) || is_object(hooks($obj.'.'.$act))) && !in_array($obj.'::'.$act, ${'denyMds_'.__TIME__})){
			...
			permissionDenied:
			WebSocket::send(json_encode(error(lang('mod.permissionDenied'))));
		}
